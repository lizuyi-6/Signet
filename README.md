...
oracle, and the whole path is covered by a Playwright acceptance smoke.
...
| `@signet/extension` | The MV3 extension: content script (scan + Shadow-DOM badge), background SW, offscreen doc (runs WASM). |
| `@signet/demo` | "Industry Intelligence Report" demo page hosting the 4 fixtures. |
...
`docs/decisions.md` (D11–D17). Per-phase progress is tracked in
...
```bash
pnpm install
pnpm verify        # typecheck + test + lint + format:check (66 tests)
pnpm test          # vitest run (66 tests)
